...
export default function TimeDashboard() {
  ...
  const trendChartData = useMemo(() => {
    ...
    selectedTopic.trend.forEach(h => {
      periodMap.set(h.period, h.paper_count);
    });
    return Array.from(periodMap.entries())
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([period, count]) => ({ period, count }));
  }, [selectedTopic, data?.periods]);

  const trendStats = useMemo(() => {
    if (!selectedTopic || selectedTopic.trend.length < 2) return null;
    const counts = selectedTopic.trend.map(h => h.paper_count);
